...
function RouteComponent() {
  ...
  const { data, isLoading, error } = useAlmanacListQuery({
    limit: PAGE_SIZE,
    ...
    <>
      <div className="flex px-2 lg:px-10 items-center">
        ...
        <Button
          ...
          onClick={() => navigate({ to: "/admin/almanac/create" })}
        >
      ...
      <div className="px-2 lg:px-10 space-y-4">
        ...
        {isLoading ? (
          ...
          <p className="text-destructive text-sm">Could not load almanacs.</p>
        ) : items.length === 0 ? (
          <p className="text-sm text-muted-foreground">No almanacs yet. Add one above.</p>
        ) : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <div
                key={item.id}
                onClick={() => navigate({ to: "/admin/almanac/$id", params: { id: item.id } })}
